...
class TextureMapDataset(Dataset):
    ...
    def load_view_dependent_data_from_disk(self, item_index, view_index):
        ...
        with Image.open(partial_texture_path) as partial_tex:
            partial_texture = np.array(partial_tex).astype(np.float32)
    ...
    def apply_batch_transforms(batch):
        ...
        batch['partial_texture'] = batch['partial_texture'] / 255 - 0.5
    ...
    @staticmethod
    def visualize_sample_pyplot(texture, normal, noc, mask_texture, render, noc_render, mask_render, partial_texture):
        ...
        noc_render = np.transpose(noc_render, (1, 2, 0))
        ...
        axarr[1, 2].axis('off')
        axarr[1, 3].imshow(partial_texture + 0.5)
        axarr[1, 3].axis('off')
        plt.tight_layout()
        plt.show()
