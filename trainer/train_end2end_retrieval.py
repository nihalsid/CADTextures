...
class TextureEnd2EndModule(pl.LightningModule):
    ...
    def step(self, batch, use_argmax=False):
        ...
        features_in, _ = self.fenc_input(batch['partial_texture'], batch['mask_missing'])
        features_tgt_normed = torch.nn.functional.normalize(self.fenc_target(self.train_dataset.unfold(batch['texture']))[0], dim=1)
        features_candidates = self.fenc_target(batch['database_textures'])[0]
    ...
    def validation_epoch_end(self, outputs):
        if int(os.environ.get('LOCAL_RANK', 0)) == 0:
            ...
            with torch.no_grad():
                candidate_codes, _ = ds_vis.get_all_texture_patch_codes(self.fenc_target, self.device, self.hparams.batch_size)
                for batch_idx, batch in enumerate(loader):
                    ds_vis.move_batch_to_gpu(batch, self.device)
                    ds_vis.apply_batch_transforms(batch)
                    features_in_unnormed, _ = self.fenc_input(batch['partial_texture'], batch['mask_missing'])
                    features_in = torch.nn.functional.normalize(features_in_unnormed, dim=1).cpu()
                    features_tgt = torch.nn.functional.normalize(self.fenc_target(ds_vis.unfold(batch['texture']))[0], dim=1)
                    ...
                    features_source_candidates, _ = TextureEnd2EndDataset.get_texture_patch_codes(self.fenc_target, source_candidates, num_patches_x * num_patches_x, self.device, torch.device('cpu'))
                    ...
                    ds_train.visualize_texture_batch_00(batch['partial_texture'].cpu().numpy(), torch.cat([batch['texture'].cpu(), retrieved_texture]).numpy(), retrieved_source_texture.numpy(), None, closest_samples, output_dir / "val_vis" / f"{batch_idx:04d}.jpg")
            ...
            features_in_attn = None
            with torch.no_grad():
                retrieved_texture, _, _ = self.step(batch, use_argmax=True)
            closest_samples = self.create_closest_textures_list(batch['name'])
            ds_train.visualize_texture_batch_00(batch['partial_texture'].cpu().numpy(), torch.cat([batch['texture'], retrieved_texture]).cpu().numpy(), None, features_in_attn.cpu().numpy() if features_in_attn is not None else None, closest_samples, output_dir / "train_batch.jpg")
