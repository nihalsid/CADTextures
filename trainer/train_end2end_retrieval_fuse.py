...
class TextureEnd2EndModule(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        features_tgt = self.fenc_target(batch['database_textures'])
    ...
    def step(self, batch, use_argmax=False):
        ...
        features_in_normed, features_tgt_normed = torch.nn.functional.normalize(features_in, dim=1), torch.nn.functional.normalize(features_tgt, dim=1)
        features_tgt_normed = features_tgt_normed.unsqueeze(0).expand(features_in_normed.shape[0], -1, -1)
        similarity_scores = torch.einsum('ik,ijk->ij', features_in_normed, features_tgt_normed)
        ...
        return TextureMapDataset.apply_mask_texture(self.fold(selected_patches), batch['mask_texture'])

    def training_step(self, batch, batch_idx):
        retrieved_texture = self.step(batch)
    ...
    def validation_epoch_end(self, outputs):
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(loader):
                ...
                features_tgt = candidate_codes.unsqueeze(0).expand(features_in.shape[0], -1, -1)
                selections = torch.argmax(torch.einsum('ik,ijk->ij', features_in, features_tgt), dim=1)
                ...
                total_loss += self.mse_loss(retrieved_texture.to(self.device), batch['texture']).cpu().item()

        total_loss /= len(ds_vis)
        self.log("val/loss_regression", total_loss, on_step=False, on_epoch=True, prog_bar=False, logger=True)
        ...
        with torch.no_grad():
            retrieved_texture = self.step(batch, use_argmax=True)
