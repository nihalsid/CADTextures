...
def quadriflow_wrapper(input_path):
    ...
    args = ["-i", f"{str(input_path / f'{res:03d}.obj')}", '-o', f"{str(input_path / output_filename)}.obj", "-f", f"{fcount}"]
...
def retopologize_file(path):
    ...
    while num_faces - N > 0:
        ...
        args = ["-i", f"{str(path.parent / f'{R:03d}.obj')}", '-o', f"{str(path.parent / output_filename)}.obj", "-f", f"{target_faces}"]
        quadriflow_executor([quad_dirname, ' '.join(args)])
        num_faces = trimesh.load(path, process=False).faces.shape[0]
...
def get_num_faces(root_files):
    ...
    res = 272
    for input_path in tqdm(root_files):
        qmeshes = [x for x in input_path.iterdir() if x.name.startswith(f'quad_{fcount:05d}_{res:03d}')]
...
def check_for_nans(root_files):
    ...
    nan_meshes = []
    for input_path in tqdm(root_files):
        meshes = [x for x in input_path.iterdir() if x.name.startswith(f'quad_{fcount:05d}_{res:03d}')]
        for m in meshes:
