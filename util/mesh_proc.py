...
def from_scratch(file_all, file_input):
    ...
    # mesh_data.edges = np.hstack([faces.T[[0, 1], :], faces.T[[1, 2], :], faces.T[[2, 0], :]])
    # cat_feats = torch.cat([torch.from_numpy(mesh_data.features.T), torch.from_numpy(mesh_data.features.T)], dim=0).float()
    # cat_idx = torch.cat([torch.from_numpy(mesh_data.edges[:, 0]), torch.from_numpy(mesh_data.edges[:, 1])], dim=0).long()
...
def get_laplacian_coordinates(mesh):
    laplacian = trimesh.smoothing.laplacian_calculation(mesh) * mesh.vertices
    return laplacian
...
def compute_face_normals_and_areas(mesh, faces):
    ...
    face_normals /= (face_areas[:, np.newaxis] + 1e-8)
