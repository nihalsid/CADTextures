...
class EncoderReduced(nn.Module):
...
class DecoderReduced(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
                 attn_resolutions, dropout=0.0, resamp_with_conv=True, in_channels,
                 resolution, z_channels, give_pre_end=False, use_tanh=True, **ignorekwargs):
        super().__init__()
...
class DecoderWithRetrieval(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
                 attn_resolutions, dropout=0.0, resamp_with_conv=True, in_channels,
                 resolution, z_channels, give_pre_end=False, use_tanh=True, **ignorekwargs):
        super().__init__()
        ...
        self.conv_ret_in = torch.nn.Conv2d(out_ch,
                                       ch,
                                       kernel_size=3,
                                       stride=1,
                                       padding=1)

        self.retrieval_features_0 = ResnetBlock(in_channels=ch,
                                                out_channels=ch,
                                                temb_channels=self.temb_ch,
                                                dropout=dropout)

        self.retrieval_features_1 = nn.Sequential(
            ResnetBlock(in_channels=ch,
                        out_channels=ch,
                        temb_channels=self.temb_ch,
                        dropout=dropout),
            ResnetBlock(in_channels=ch,
                        out_channels=ch*ch_mult[1],
                        temb_channels=self.temb_ch,
                        dropout=dropout),
        )
    ...
    def forward(self, z, retrieval):
        ...
        ret_feats = self.conv_ret_in(retrieval)
        ret_feats = [self.retrieval_features_0(ret_feats), self.retrieval_features_1(ret_feats)]
...
class DeformableEncoder(nn.Module):
    ...
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        # end
        self.deform_conv = DeformConv2d(block_in, block_in, kernel_size=3, stride=1, padding=1)
        self.deform_offset = torch.nn.Conv2d(block_in, 3 * 3 * 2, kernel_size=3, stride=1, padding=1, bias=False)
        self.deform_norm = Normalize(block_in)
    ...
    def forward(self, x):
        ...
        # deform
        offsets = self.deform_offset(h)
        h = self.deform_conv(h, offsets)
        h = self.deform_norm(h)
        h = nonlinearity(h)
